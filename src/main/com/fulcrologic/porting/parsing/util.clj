...
(>defn sym->fqsym
  ...
  (let [feature         (log/spy :info (:feature-context env))
        {:keys [nsalias->ns raw-sym->fqsym]} (log/spy :info (get-in env [:parsing-envs feature]))
        potential-alias (some-> (log/spy :info sym) namespace symbol)
        real-ns         (get nsalias->ns (log/spy :info potential-alias))
        fqsym           (if real-ns
